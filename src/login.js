...
function Login() {
  ...
  const Login = () =>{
    ...
    axios.post("http://stawro.xyz/google/loginn" , {email})
    .then(res => {
      if(res.data.Status === "OK"){
        axios.post('http://stawro.xyz/api/check/nme')
        .then(res =>{
          if(res.data.Status === "BAD"){
            const username = localStorage.getItem("unam");
            const name = localStorage.getItem("name");
            const picture = localStorage.getItem("picture");
            axios.post('http://stawro.xyz/userinfo',{name, username, email, picture})
            .then(res =>{
              if(res.data.Status === "OK"){
        ...
        alert("sorry Darshan I can't Login you want to still fix some error")
      }
    })
  }
