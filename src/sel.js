...
const Sel = () => {
  ...
  const HandleSubmit = async (e) => {
    e.preventDefault();
    axios.post('http://stawro.xyz/way/email', {email})
    .then(res => {
      if(res.data.Status === "BAD"){
        axios.post('http://stawro.xyz/validtr/id', {trID})
        .then(res => {
          if(res.data.Status === "BAD"){
            e.preventDefault();
        axios.post('http://stawro.xyz/trid/id', {trID})
        .then(res => {
          if(res.data.Status === "OK"){
            axios.post("http://stawro.xyz/select",{way,email, ip})
            .then(res => {
            if(res.data.Status === "OK") {
              axios.post("http://stawro.xyz/way01",{way,email, ip})
              .then(resp => {
                if(resp.data.Status === "OK"){
                  axios.post('http://stawro.xyz/validid/post',{email, valid, trID})
                  .then(ress => {
